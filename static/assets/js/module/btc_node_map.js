...
// Create map instance
var chart = am4core.create("amnodemap", am4maps.MapChart);

var title = chart.titles.create();
title.text = "[bold font-size: 20]Bitcoin Node Distribution Map[/]\nsource: darkbee";
title.textAlign = "middle";

// 维度 精度
var latlong = {
    "beijing":{"latitude":39.9, "longitude":116.3},
...
var mapData = [
  ...
  { "id":"CN", "name":"China", "value":0, "color": chart.colors.getIndex(0) },
...
var orgin_data=btc_ct_series;
var last_data=[];
console.log(orgin_data)
    for(var i=0;i<orgin_data.length-1;i++){
    var temp_name=orgin_data[i].name;
    var temp_value=orgin_data[i].value;
    for(var j=0;j<mapData.length;j++){
      if (mapData[j].name==temp_name)
      {
          var temp_id=mapData[j].id;
          var latitude=latlong[temp_id].latitude;
          var longitude=latlong[temp_id].longitude;
      }else {
        continue
      }
    }
    ...
    temp={ "id":temp_id, "name":temp_name, "value":temp_value, "color": chart.colors.getIndex(0),"latitude":latitude,"longitude":longitude};
    last_data.push(temp);
    }
